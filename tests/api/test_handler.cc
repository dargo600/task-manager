...
TEST_F(HandlerTest, GeneratesResponseForMarkdown) {
    ...
    EXPECT_CALL(mockParser,
                create_html(md_filename, html_filename, ::testing::_))
        .WillOnce(::testing::Invoke([](const std::string& /*md*/,
    ...
    http::request<http::string_body> req{http::verb::get, get_html_filename,
                                         version};
...
TEST_F(HandlerTest, ReadsHTMLOK) {
    api::MockMarkDownParser mockParser;
    Handler handler(boost::beast::http::status::ok, 11, mockParser);
    const std::string md_filename = "res/test.md";
    const std::string html_filename = "res/test.html";
    ...
    std::ofstream ofs;
    ofs.open(html_filename);
    ofs << "<h1>Test Title</h1>\n<p>This is a test.</p>";
    ofs.close();
...
TEST_F(HandlerTest, TestInValidRequest) {
    api::MockMarkDownParser mockParser;
    Handler handler(boost::beast::http::status::ok, 11, mockParser);
    const std::string get_html_filename = "/some/test.html";
    int version = 11;  // http version 1.1
    http::request<http::string_body> req{http::verb::get, get_html_filename,
                                         version};

    auto valid_request = handler.is_valid_request(req);
    EXPECT_EQ(valid_request, false);
}

TEST_F(HandlerTest, TestValidRequest) {
    api::MockMarkDownParser mockParser;
    Handler handler(boost::beast::http::status::ok, 11, mockParser);
    const std::string get_html_filename = "/data/test.html";
    int version = 11;  // http version 1.1
